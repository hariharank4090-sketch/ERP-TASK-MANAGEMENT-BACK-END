...
function extractRoutes(appOrRouter, base = '') {
...
function buildGroupTree(routes) {
    const root = { name: 'api', fullPath: '/api', children: new Map(), apis: [] };
...
function renderHTML(root, total) {
    const head = `
  <!doctype html>
  <html lang="en">
  <head>
...
function renderGroup(node, isRoot = false) {
