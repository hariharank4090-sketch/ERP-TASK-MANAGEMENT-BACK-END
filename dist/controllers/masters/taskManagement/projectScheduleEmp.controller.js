...
exports.updateTaskDetailsBulk = exports.createTaskDetailsRaw = exports.getTaskDetailsStatistics = exports.deleteTaskDetail = exports.updateTaskDetail = exports.getTaskDetailsWithFilters = exports.getTaskDetailsByEmployee = exports.getTaskDetailsByTask = exports.getTaskDetailsBySchedule = exports.getTaskDetailsByProject = exports.getTaskDetailById = exports.getAllTaskDetails = void 0;
const sequelize_1 = require("sequelize");
const zod_1 = require("zod");
const responseObject_1 = require("../../../responseObject");
const type_model_1 = require("../../../models/taskDetails/type.model");
...
function mapRawToTaskDetail(raw) {
    if (!raw)
        return {};
    return {
        Id: raw.Id,
...
const getCompanyDB = (req) => {
    return req.companyDB || req.sequelize;
};
// Helper to get TaskDetail model for specific database
const getTaskDetailModel = (sequelizeInstance) => {
    return (0, type_model_1.initTaskDetailModel)(sequelizeInstance);
};
const getAllTaskDetails = async (req, res) => {
    try {
        const companyDB = getCompanyDB(req);
        const validation = validateWithZod(type_model_1.taskDetailQuerySchema, req.query);
        ...
        const { Project_Id, Sch_Id, Task_Id, Emp_Id, Assigned_Emp_Id, Invovled_Stat, from_Task_Assign_dt, to_Task_Assign_dt, sortBy = 'td.Id', sortOrder = 'DESC' } = validation.data;
        ...
        const rows = await companyDB.query(`WITH LatestSchedules AS (
...
exports.getAllTaskDetails = getAllTaskDetails;
const getTaskDetailById = async (req, res) => {
    try {
        const companyDB = getCompanyDB(req);
        ...
        const validation = validateWithZod(type_model_1.taskDetailIdSchema, { id });
        if (!validation.success) {
            return res.status(400).json({
                success: false,
                message: 'Validation failed',
        ...
        const rows = await companyDB.query(`SELECT 
                td.*,
                ps.Task_Sch_Timer_Based as Schedule_Task_Sch_Timer_Based,
                ps.Sch_No as Schedule_Sch_No,
                ps.Sch_Date as Schedule_Sch_Date,
        ...
        if (!taskDetail) {
            return (0, responseObject_1.notFound)(res, 'Task detail not found');
        }
...
const getTaskDetailsByProject = async (req, res) => {
    try {
        const companyDB = getCompanyDB(req);
...
exports.getTaskDetailsByProject = getTaskDetailsByProject;
const getTaskDetailsBySchedule = async (req, res) => {
    try {
        const companyDB = getCompanyDB(req);
...
exports.getTaskDetailsBySchedule = getTaskDetailsBySchedule;
const getTaskDetailsByTask = async (req, res) => {
    try {
        const companyDB = getCompanyDB(req);
...
exports.getTaskDetailsByTask = getTaskDetailsByTask;
const getTaskDetailsByEmployee = async (req, res) => {
    try {
        const companyDB = getCompanyDB(req);
...
exports.getTaskDetailsByEmployee = getTaskDetailsByEmployee;
const getTaskDetailsWithFilters = async (req, res) => {
    try {
        const companyDB = getCompanyDB(req);
        const validation = validateWithZod(type_model_1.taskDetailQuerySchema, req.query);
        if (!validation.success) {
        ...
        const { Project_Ids, Task_Ids, Emp_Ids, has_AN_No, has_Assigned_Emp, sortBy = 'td.Id', sortOrder = 'DESC' } = validation.data;
        let whereClause = '1=1';
        const replacements = {};
...
exports.getTaskDetailsWithFilters = getTaskDetailsWithFilters;
const updateTaskDetail = async (req, res) => {
    try {
        ...
        const idValidation = validateWithZod(type_model_1.taskDetailIdSchema, { id });
        ...
        const bodyValidation = validateWithZod(type_model_1.taskDetailUpdateSchema, req.body);
        ...
        if (updateData.Sch_Id || updateData.Emp_Id || updateData.Task_Assign_dt) {
            ...
            const duplicateCheck = await companyDB.query(`SELECT Id FROM tbl_Task_Details 
                 WHERE Sch_Id = :schId 
                   AND Emp_Id = :empId 
        ...
        if (!updatedTaskDetail) {
            return (0, responseObject_1.notFound)(res, 'Task detail not found after update');
        }
...
exports.deleteTaskDetail = deleteTaskDetail;
const getTaskDetailsStatistics = async (req, res) => {
    try {
        const companyDB = getCompanyDB(req);
        const TaskDetailModel = getTaskDetailModel(companyDB);
        const totalRecords = await TaskDetailModel.count();
        const recordsWithAN = await TaskDetailModel.count({ where: { AN_No: { [sequelize_1.Op.ne]: null } } });
        const recordsWithAssignedEmp = await TaskDetailModel.count({ where: { Assigned_Emp_Id: { [sequelize_1.Op.ne]: null } } });
        const statusCounts = await TaskDetailModel.findAll({
            ...
            where: { Invovled_Stat: { [sequelize_1.Op.ne]: null } },
            group: ['Invovled_Stat']
        });
        const timerBasedStatsResult = await companyDB.query(`SELECT 
             ...
             FROM tbl_Task_Details td
             LEFT JOIN tbl_Project_Schedule ps ON td.Task_Id = ps.Task_Id`, { type: 'SELECT' });
...
exports.getTaskDetailsStatistics = getTaskDetailsStatistics;
const createTaskDetailsRaw = async (req, res) => {
    const companyDB = getCompanyDB(req);
    const transaction = await companyDB.transaction();
    try {
        const validation = validateWithZod(type_model_1.taskDetailCreateSchema, req.body);
        if (!validation.success) {
        ...
        const { Project_Id, Sch_Id, Task_Id, Emp_Ids, Task_Levl_Id, Assigned_Emp_Id, Ord_By, Invovled_Stat } = validation.data;
        // Fetch data from tbl_Project_Sch_Task_DT based on Sch_Id
        const scheduleTaskData = await companyDB.query(`SELECT 
                Task_Work_Date,
                Task_Start_Time,
                Task_End_Time
        ...
        const maxAnNoResult = await companyDB.query(`SELECT ISNULL(MAX(AN_No), 0) AS maxANNo FROM tbl_Task_Details`, { transaction, type: 'SELECT' });
        let nextANNo = (maxAnNoResult[0]?.maxANNo || 0) + 1;
        const insertedAN_NoValues = [];
        ...
        for (let i = 0; i < Emp_Ids.length; i++) {
            for (let j = 0; j < scheduleTaskData.length; j++) {
                const taskRecord = scheduleTaskData[j];
                // Check if duplicate exists for Sch_Id, Emp_Id, and Task_Assign_dt (date only)
                const checkDuplicate = await companyDB.query(`SELECT Id FROM tbl_Task_Details 
                     WHERE Sch_Id = :schId 
                       AND Emp_Id = :empId 
                       AND Task_Assign_dt >= CAST(:taskAssignDt AS DATETIME)
                   AND Task_Assign_dt < DATEADD(day, 1, CAST(:taskAssignDt AS DATETIME))`, {
                ...
                await companyDB.query(`INSERT INTO tbl_Task_Details
        ...
        return res.status(201).json({
            ...
            data: {
                ...
                employeeIds: Emp_Ids,
                scheduleTaskDates: scheduleTaskData.map((record) => ({
...
exports.createTaskDetailsRaw = createTaskDetailsRaw;
const updateTaskDetailsBulk = async (req, res) => {
    const companyDB = getCompanyDB(req);
    const transaction = await companyDB.transaction();
    try {
        ...
        const existingRecords = await companyDB.query(`SELECT Id, Task_Assign_dt FROM tbl_Task_Details 
        ...
        const duplicateRecords = [];
        const scheduleDates = scheduleTaskData.map((record) => new Date(record.Task_Work_Date).toISOString().split('T')[0]);
        ...
        if (recordsToDelete.length > 0) {
            ...
            await companyDB.query(`DELETE FROM tbl_Task_Details WHERE Id IN (${placeholders})`, {
        ...
        for (let j = 0; j < scheduleTaskData.length; j++) {
            ...
            if (existingDatesMap.has(recordDate)) {
                ...
                await companyDB.query(`UPDATE tbl_Task_Details 
...
exports.updateTaskDetailsBulk = updateTaskDetailsBulk;
