...
exports.getAllActiveSchedules = exports.getScheduleDropdown = exports.getScheduleTypesDropdown = exports.getTasksDropdown = exports.getSchedulePlansDropdown = exports.deleteSchedule = exports.updateScheduleStatus = exports.updateSchedule = exports.createSchedule = exports.getScheduleDetails = exports.getScheduleById = exports.getAllSchedules = void 0;
const sequelize_1 = require("sequelize");
const zod_1 = require("zod");
const responseObject_1 = require("../../../responseObject");
const schedule_type_model_1 = require("../../../models/masters/ProjectSchedule/schedule.type.model");
...
async function getNextAId(db, transaction) {
    const maxAIdResult = await db.query(`SELECT ISNULL(MAX(A_Id), 0) + 1 as NextId FROM tbl_Project_Sch_Task_DT`, { type: sequelize_1.QueryTypes.SELECT, transaction });
    let nextAId = maxAIdResult[0]?.NextId || 1;
    const aIdCheck = await db.query(`SELECT 1 FROM tbl_Project_Sch_Task_DT WHERE A_Id = ?`, { replacements: [nextAId], type: sequelize_1.QueryTypes.SELECT, transaction });
    if (aIdCheck.length > 0) {
        const availableAIdResult = await db.query(`SELECT TOP 1 t1.A_Id + 1 as AvailableId
             FROM tbl_Project_Sch_Task_DT t1
             LEFT JOIN tbl_Project_Sch_Task_DT t2 ON t1.A_Id + 1 = t2.A_Id
             WHERE t2.A_Id IS NULL
             ORDER BY t1.A_Id`, { type: sequelize_1.QueryTypes.SELECT, transaction });
        nextAId = availableAIdResult[0]?.AvailableId || nextAId + 1;
    }
    return nextAId;
}
async function insertTaskDates(db, schId, dates, estStartTime, estEndTime, transaction) {
    for (const date of dates) {
        const nextAId = await getNextAId(db, transaction);
        await db.query(`INSERT INTO tbl_Project_Sch_Task_DT
...
async function insertPlanDetails(db, schId, planId, selectedDays, planDetails, transaction) {
    ...
    for (const day of selectedDays) {
        await db.query(`INSERT INTO tbl_Project_Sch_DT (Sch_Id, Plan_Month, Plan_Day)
             VALUES (?, ?, ?)`, {
            replacements: [
                schId,
...
function generateTaskDates(startDate, endDate, planId, selectedDays, _planDetails, _specificDates = []) {
    const dates = [];
...
const getAllSchedules = async (req, res) => {
    try {
        ...
        const validation = validateWithZod(schedule_type_model_1.ScheduleQuerySchema, req.query);
        ...
        if (sortBy === 'Sch_No')
            sortColumn = 's.Sch_No';
        else if (sortBy === 'Sch_Date')
            sortColumn = 's.Sch_Date';
        else if (sortBy === 'Task_Name')
            sortColumn = 't.Task_Name';
        else if (sortBy === 'Sch_Status')
            sortColumn = 's.Sch_Status';
        else if (sortBy === 'Entry_Date')
            sortColumn = 's.Entry_Date';
        ...
        const [countResult, schedules] = await Promise.all([
            db.query(countQuery, { replacements, type: sequelize_1.QueryTypes.SELECT }),
            db.query(schedulesQuery, { replacements, type: sequelize_1.QueryTypes.SELECT })
        ]);
        ...
        const [taskDates, planDetails] = await Promise.all([
            db.query(taskDatesQuery, { type: sequelize_1.QueryTypes.SELECT }),
            db.query(planDetailsQuery, { type: sequelize_1.QueryTypes.SELECT })
        ]);
        const taskDatesByScheduleId = {};
        const planDetailsByScheduleId = {};
...
const getScheduleById = async (req, res) => {
    try {
        ...
        const validation = validateWithZod(schedule_type_model_1.ScheduleIdSchema, req.params);
        if (!validation.success) {
        ...
        `;
        const rows = await db.query(query, { replacements: [validation.data.id], type: sequelize_1.QueryTypes.SELECT });
        if (!rows.length)
            return (0, responseObject_1.notFound)(res, 'Project schedule not found');
        const detailsQuery = `SELECT * FROM tbl_Project_Sch_Task_DT WHERE Sch_Id = ? ORDER BY Task_Work_Date`;
        const details = await db.query(detailsQuery, { replacements: [validation.data.id], type: sequelize_1.QueryTypes.SELECT });
        const planDtQuery = `SELECT * FROM tbl_Project_Sch_DT WHERE Sch_Id = ?`;
        const planDt = await db.query(planDtQuery, { replacements: [validation.data.id], type: sequelize_1.QueryTypes.SELECT });
        (0, responseObject_1.sentData)(res, { ...rows[0], taskDates: details, planDetails: planDt });
    }
    catch (e) {
...
const getScheduleDetails = async (req, res) => {
    try {
        ...
        const rows = await db.query(query, { replacements: [validation.data.id], type: sequelize_1.QueryTypes.SELECT });
        (0, responseObject_1.sentData)(res, rows);
    }
    catch (e) {
...
exports.getScheduleDetails = getScheduleDetails;
...
const createSchedule = async (req, res) => {
    ...
    try {
        ...
        const validation = validateWithZod(schedule_type_model_1.ScheduleCreateSchema, body);
        if (!validation.success) {
            console.log('Validation Errors:', JSON.stringify(validation.errors, null, 2));
            return res.status(400).json({
                success: false,
        ...
        const { Sch_No, Sch_Date, Task_Id, Task_Type_Id, Sch_Plan_Id, Sch_Type, Task_Sch_Timer_Based, Sch_Est_Start_Time, Sch_Est_End_Time, Task_Sch_Duaration, Sch_Status, Entry_By, planDetails, selectedDays, specificDates } = validation.data;
        let Sch_Start_Date = validation.data.Sch_Start_Date;
        let Sch_End_Date = validation.data.Sch_End_Date;
        // PLAN 5 => AUTO SET START/END DATE FROM specificDates
        ...
        // Check for duplicate schedule number
        const dupCheck = await db.query(`SELECT 1 FROM tbl_Project_Schedule WHERE UPPER(Sch_No) = UPPER(?) AND Sch_Del_Flag = 0`, { replacements: [Sch_No], type: sequelize_1.QueryTypes.SELECT });
        if (dupCheck.length) {
            return res.status(409).json({
                success: false,
                message: 'Schedule number already exists'
        ...
        try {
            ...
            const maxIdResult = await db.query(`SELECT ISNULL(MAX(Sch_Id), 0) + 1 as NextId
                 FROM tbl_Project_Schedule WITH (TABLOCKX, HOLDLOCK)`, { type: sequelize_1.QueryTypes.SELECT, transaction });
            const nextSchId = maxIdResult[0]?.NextId;
            if (!nextSchId)
            ...
            await db.query(`INSERT INTO tbl_Project_Schedule
                 (Sch_Id, Sch_No, Sch_Date, Task_Id, Task_Type_Id, Sch_Plan_Id,
                  Sch_Type,
                  Sch_Start_Date, Sch_End_Date, Task_Sch_Timer_Based,
                  Sch_Est_Start_Time, Sch_Est_End_Time, Task_Sch_Duaration,
            ...
            if (Sch_Plan_Id === 5) {
                ...
                if (specificDates && specificDates.length > 0) {
                    taskDatesToInsert = specificDates
                        .filter(ds => ds && /^\d{4}-\d{2}-\d{2}$/.test(ds))
                        .map(ds => parseDateString(ds));
                    console.log(`Plan 5: Using ${taskDatesToInsert.length} specific dates from frontend (IGNORING Sch_Start_Date/Sch_End_Date):`, taskDatesToInsert.map(d => formatDateForSQL(d)));
                }
                ...
                taskDatesToInsert = generateTaskDates(new Date(Sch_Start_Date), new Date(Sch_End_Date), Sch_Plan_Id, selectedDays || [], planDetails, specificDates || []);
                console.log(`Plan ${Sch_Plan_Id}: Generated ${taskDatesToInsert.length} task dates from range`);
            }
            if (taskDatesToInsert.length > 0) {
            ...
            isTransactionActive = false;
            transaction = null;
            const data = await db.query(`SELECT s.*, t.Task_Name, p.Plan_Type, pm.Project_Name, pm.Project_Id
                 FROM tbl_Project_Schedule s
                 LEFT JOIN tbl_Task t ON s.Task_Id = t.Task_Id
                 LEFT JOIN tbl_Project_Master pm ON t.Project_Id = pm.Project_Id
                 LEFT JOIN tbl_Sch_Plan p ON s.Sch_Plan_Id = p.Plan_Id
                 WHERE s.Sch_Id = ?`, { replacements: [nextSchId], type: sequelize_1.QueryTypes.SELECT });
            const taskDatesResult = await db.query(`SELECT * FROM tbl_Project_Sch_Task_DT WHERE Sch_Id = ? ORDER BY Task_Work_Date`, { replacements: [nextSchId], type: sequelize_1.QueryTypes.SELECT });
            const planDt = Sch_Plan_Id !== 5
                ? await db.query(`SELECT * FROM tbl_Project_Sch_DT WHERE Sch_Id = ?`, { replacements: [nextSchId], type: sequelize_1.QueryTypes.SELECT })
                : [];
            console.log('Final task dates saved to tbl_Project_Sch_Task_DT:', taskDatesResult.map(t => t.Task_Work_Date));
            console.log('Final schedule saved to tbl_Project_Schedule:', data[0]?.Sch_Id, data[0]?.Sch_No);
        ...
        const original = e?.original ?? e?.parent;
        ...
        if (transaction && isTransactionActive) {
            try {
                ...
                console.error('Final rollback error:', err);
            }
        }
        const resolvedMessage = (e instanceof Error && e.message) ||
            original?.message ||
...
const updateSchedule = async (req, res) => {
    ...
    try {
        ...
        const idCheck = validateWithZod(schedule_type_model_1.ScheduleIdSchema, req.params);
        if (!idCheck.success) {
            return res.status(400).json({ success: false, errors: idCheck.errors });
        }
        const bodyCheck = validateWithZod(schedule_type_model_1.ScheduleUpdateSchema, req.body);
        ...
        const exists = await db.query(`SELECT Sch_Plan_Id, Sch_Status FROM tbl_Project_Schedule WHERE Sch_Id = ? AND Sch_Del_Flag = 0`, { replacements: [scheduleId], type: sequelize_1.QueryTypes.SELECT });
        if (!exists.length)
            return (0, responseObject_1.notFound)(res, 'Project schedule not found');
        ...
        if (updateData.Sch_No) {
            const dupCheck = await db.query(`SELECT 1 FROM tbl_Project_Schedule
                 WHERE UPPER(Sch_No) = UPPER(?) AND Sch_Id != ? AND Sch_Del_Flag = 0`, { replacements: [updateData.Sch_No, scheduleId], type: sequelize_1.QueryTypes.SELECT });
            if (dupCheck.length) {
                return res.status(409).json({
                    success: false,
                    message: 'Schedule number already exists'
        ...
        try {
            ...
            if (updateData.Task_Type_Id !== undefined) {
                updateFields.push('Task_Type_Id = ?');
                updateValues.push(updateData.Task_Type_Id);
            }
            if (updateData.Sch_Plan_Id !== undefined) {
                updateFields.push('Sch_Plan_Id = ?');
                updateValues.push(updateData.Sch_Plan_Id);
            }
            if (updateData.Sch_Type !== undefined) {
                updateFields.push('Sch_Type = ?');
                updateValues.push(updateData.Sch_Type);
            }
            ...
            if (updateFields.length > 0) {
                ...
                await db.query(`UPDATE tbl_Project_Schedule SET ${updateFields.join(', ')} WHERE Sch_Id = ?`, { replacements: updateValues, transaction });
            }
            ...
            if (shouldRegenerate) {
                ...
                const getCurrentValue = async (field) => {
                    const result = await db.query(`SELECT ${field} FROM tbl_Project_Schedule WHERE Sch_Id = ?`, { replacements: [scheduleId], type: sequelize_1.QueryTypes.SELECT, transaction });
                ...
                if (planId === 5) {
                    ...
                    taskDatesToInsert = generateTaskDates(new Date(startDate), new Date(endDate), planId, selectedDaysArray, updateData.planDetails, []);
                }
                if (taskDatesToInsert.length > 0) {
            ...
            const data = await db.query(`SELECT s.*, t.Task_Name, p.Plan_Type, pm.Project_Name, pm.Project_Id
                 ...
                 LEFT JOIN tbl_Project_Master pm ON t.Project_Id = pm.Project_Id
                 LEFT JOIN tbl_Sch_Plan p ON s.Sch_Plan_Id = p.Plan_Id
                 WHERE s.Sch_Id = ?`, { replacements: [scheduleId], type: sequelize_1.QueryTypes.SELECT });
            const taskDatesResult = await db.query(`SELECT * FROM tbl_Project_Sch_Task_DT WHERE Sch_Id = ? ORDER BY Task_Work_Date`, { replacements: [scheduleId], type: sequelize_1.QueryTypes.SELECT });
            const planDt = await db.query(`SELECT * FROM tbl_Project_Sch_DT WHERE Sch_Id = ?`, { replacements: [scheduleId], type: sequelize_1.QueryTypes.SELECT });
            return (0, responseObject_1.updated)(res, { ...data[0], taskDates: taskDatesResult, planDetails: planDt }, 'Project schedule updated successfully');
        }
        catch (error) {
            console.error('Error during transaction operations:', error);
            if (transaction && isTransactionActive) {
...
const updateScheduleStatus = async (req, res) => {
    try {
        ...
        const bodyCheck = validateWithZod(schedule_type_model_1.ScheduleStatusUpdateSchema, req.body);
        if (!bodyCheck.success) {
            return res.status(400).json({ success: false, errors: bodyCheck.errors });
        }
        ...
        const currentRecord = await db.query(`SELECT Sch_Status FROM tbl_Project_Schedule WHERE Sch_Id = ? AND Sch_Del_Flag = 0`, { replacements: [scheduleId], type: sequelize_1.QueryTypes.SELECT });
        ...
        await db.query(`UPDATE tbl_Project_Schedule 
             SET Sch_Status = ?, 
                 Update_By = ?, 
                 Update_Date = GETDATE()
                 ${compDateUpdate}
             WHERE Sch_Id = ? AND Sch_Del_Flag = 0`, { replacements: [status, updateBy, scheduleId] });
        // Fetch updated record to return
        const updatedRecord = await db.query(`SELECT s.*, t.Task_Name, p.Plan_Type, pm.Project_Name, pm.Project_Id
             FROM tbl_Project_Schedule s
             LEFT JOIN tbl_Task t ON s.Task_Id = t.Task_Id
             LEFT JOIN tbl_Project_Master pm ON t.Project_Id = pm.Project_Id
             LEFT JOIN tbl_Sch_Plan p ON s.Sch_Plan_Id = p.Plan_Id
             WHERE s.Sch_Id = ? AND s.Sch_Del_Flag = 0`, { replacements: [scheduleId], type: sequelize_1.QueryTypes.SELECT });
        ...
        (0, responseObject_1.updated)(res, responseData, 'Schedule status updated successfully');
    }
    catch (e) {
...
const deleteSchedule = async (req, res) => {
    try {
        ...
        await db.query(`UPDATE tbl_Project_Schedule SET Sch_Del_Flag = 1, Update_Date = GETDATE()
             WHERE Sch_Id = ? AND Sch_Del_Flag = 0`, { replacements: [validation.data.id] });
        (0, responseObject_1.deleted)(res, 'Project schedule deleted successfully');
    }
    catch (e) {
...
const getSchedulePlansDropdown = async (req, res) => {
    try {
        const db = getDb(req);
        const currentDBName = getCurrentDatabaseName(req);
        console.log(`📊 Getting schedule plans dropdown from database: ${currentDBName}`);
        const rows = await db.query(`SELECT Plan_Id as value, Plan_Type as label FROM tbl_Sch_Plan ORDER BY Plan_Id`, { type: sequelize_1.QueryTypes.SELECT });
        (0, responseObject_1.sentData)(res, rows);
    }
    catch (e) {
...
exports.getSchedulePlansDropdown = getSchedulePlansDropdown;
const getTasksDropdown = async (req, res) => {
    try {
        const db = getDb(req);
        const currentDBName = getCurrentDatabaseName(req);
        console.log(`📊 Getting tasks dropdown from database: ${currentDBName}`);
        const rows = await db.query(`SELECT Task_Id as value, Task_Name as label FROM tbl_Task ORDER BY Task_Name`, { type: sequelize_1.QueryTypes.SELECT });
        (0, responseObject_1.sentData)(res, rows);
    }
    catch (e) {
...
exports.getTasksDropdown = getTasksDropdown;
const getScheduleTypesDropdown = async (req, res) => {
    try {
        const scheduleTypes = [
        ...
        (0, responseObject_1.sentData)(res, scheduleTypes);
    }
    catch (e) {
...
exports.getScheduleTypesDropdown = getScheduleTypesDropdown;
const getScheduleDropdown = async (req, res) => {
    try {
        const db = getDb(req);
        const currentDBName = getCurrentDatabaseName(req);
        console.log(`📊 Getting schedule dropdown from database: ${currentDBName}`);
        const rows = await db.query(`SELECT s.Sch_Id as value, s.Sch_No + ' - ' + ISNULL(t.Task_Name, 'No Task') as label
             FROM tbl_Project_Schedule s
             LEFT JOIN tbl_Task t ON s.Task_Id = t.Task_Id
             WHERE s.Sch_Del_Flag = 0
             ORDER BY s.Sch_No DESC`, { type: sequelize_1.QueryTypes.SELECT });
        (0, responseObject_1.sentData)(res, rows);
    }
    catch (e) {
...
exports.getScheduleDropdown = getScheduleDropdown;
const getAllActiveSchedules = async (req, res) => {
    try {
        const db = getDb(req);
        const currentDBName = getCurrentDatabaseName(req);
        console.log(`📊 Getting active schedules from database: ${currentDBName}`);
        const rows = await db.query(`SELECT s.*, t.Task_Name, p.Plan_Type, pm.Project_Name, pm.Project_Id
             FROM tbl_Project_Schedule s
             LEFT JOIN tbl_Task t ON s.Task_Id = t.Task_Id
             LEFT JOIN tbl_Project_Master pm ON t.Project_Id = pm.Project_Id
             LEFT JOIN tbl_Sch_Plan p ON s.Sch_Plan_Id = p.Plan_Id
             WHERE s.Sch_Del_Flag = 0 AND s.Sch_Status = 1
             ORDER BY s.Sch_Date DESC`, { type: sequelize_1.QueryTypes.SELECT });
        (0, responseObject_1.sentData)(res, rows);
    }
    catch (e) {
...
exports.getAllActiveSchedules = getAllActiveSchedules;
