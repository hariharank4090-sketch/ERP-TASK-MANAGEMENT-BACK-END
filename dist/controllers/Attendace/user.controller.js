...
exports.changePassword = exports.getUserDropdown = exports.restoreUser = exports.hardDeleteUser = exports.deleteUser = exports.updateUser = exports.createUser = exports.getUserById = exports.getAllUsers = void 0;
const sequelize_1 = require("sequelize");
const zod_1 = require("zod");
const sequelize_2 = require("sequelize");
const bcrypt_1 = __importDefault(require("bcrypt"));
const User_model_1 = require("../../models/Attendance/user/User.model");
// Define the User class that extends Model
class User extends sequelize_2.Model {
    static async getNextId(sequelize) {
        try {
            const result = await sequelize.query('SELECT ISNULL(MAX(UserId), 0) + 1 as nextId FROM tbl_Users WHERE UserTypeId = 6', {
                type: 'SELECT',
                raw: true
            });
...
const getAllUsers = async (req, res) => {
    try {
        ...
        const validation = validateWithZod(User_model_1.userQuerySchema, req.query);
        ...
        const { page = 1, limit = 20, search, sortBy = 'UserId', sortOrder = 'ASC', companyId, branchId, udelFlag } = validation.data;
        const where = {
        ...
        if (search && search.trim()) {
            where[sequelize_1.Op.or] = [
                ...
                { Global_User_ID: { [sequelize_1.Op.like]: `%${search}%` } }
            ];
        }
...
const getUserById = async (req, res) => {
    try {
        const sequelizeInstance = getSequelizeFromRequest(req);
        const UserModel = initializeUserModel(sequelizeInstance);
        const validation = validateWithZod(User_model_1.userIdSchema, req.params);
        if (!validation.success) {
            return res.status(400).json({
                success: false,
...
const createUser = async (req, res) => {
    try {
        ...
        const validation = validateWithZod(User_model_1.userCreateSchema, req.body);
        if (!validation.success) {
            return res.status(400).json({
                success: false,
                message: 'Validation failed',
...
const updateUser = async (req, res) => {
    try {
        ...
        const idValidation = validateWithZod(User_model_1.userIdSchema, req.params);
        ...
        const bodyValidation = validateWithZod(User_model_1.userUpdateSchema, req.body);
        ...
        if (bodyValidation.data.UserName) {
            const duplicate = await UserModel.findOne({
                where: {
                    ...
                    UserId: { [sequelize_1.Op.ne]: user.UserId },
                    UserTypeId: 6
                }
            });
            if (duplicate) {
...
const getUserDropdown = async (req, res) => {
    try {
        ...
        if (req.query.companyId) {
            where.Company_Id = parseInt(req.query.companyId);
        }
