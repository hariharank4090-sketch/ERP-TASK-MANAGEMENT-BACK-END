...
exports.getLOS = exports.getLOL = exports.getLargeData = exports.getNextId = exports.getProducts = exports.getRetailerInfo = exports.getUserMenuRights = exports.getUserBasedRights = exports.getUserTypeBasedRights = exports.getUserIdByAuth = exports.getCustomerIdByUserId = exports.getUserTypeByAuth = exports.getUserType = exports.miniResponse = exports.isEqualNumber = exports.checkIsNumber = void 0;
...
const isEqualNumber = (value1, value2) => {
...
const miniResponse = ({ status = true, dataArray = [], dataObject = {}, others = {} }) => ({
    status,
    dataArray,
    dataObject,
    ...others
});
exports.miniResponse = miniResponse;
const getUserType = async (UserId) => {
    if (!(0, exports.checkIsNumber)(UserId)) {
        return false;
    }
    try {
...
const getUserTypeByAuth = async (Auth) => {
    if (!Auth) {
        return false;
    }
    try {
        const request = new mssql_1.default.Request();
        request.input('Auth', mssql_1.default.NVarChar, Auth);
        const result = await request.query(`
            SELECT ut.Id
            FROM 
                tbl_Users AS u
...
exports.getCustomerIdByUserId = getCustomerIdByUserId;
const getUserIdByAuth = async (Auth) => {
    if (!Auth) {
        return false;
    }
    try {
...
const getUserTypeBasedRights = async (usertype) => {
    if (!userPortalDB) {
    ...
    try {
        const request = new mssql_1.default.Request();
        request.input('usertype', mssql_1.default.Int, usertype);
        const result = await request.query(`
            SELECT 
                m.*,
...
const getUserBasedRights = async (userid) => {
    if (!(0, exports.checkIsNumber)(userid)) {
        return false;
    }
    if (!userPortalDB) {
...
const getUserMenuRights = async (Auth) => {
    if (!userPortalDB) {
        console.error('USERPORTALDB environment variable is not defined');
        return false;
    }
    try {
        const UserTypeId = await (0, exports.getUserTypeByAuth)(Auth);
        ...
        if ((0, exports.isEqualNumber)(UserTypeId, 0) || (0, exports.isEqualNumber)(UserTypeId, 1)) {
            ...
            const UserId = await (0, exports.getUserIdByAuth)(Auth);
            if (!UserId) {
                return false;
            }
            const request = new mssql_1.default.Request();
            request.input('userid', mssql_1.default.Int, UserId);
            request.input('usertype', mssql_1.default.Int, UserTypeId);
            const result = await request.query(`
                SELECT 
                    m.*,
...
const getRetailerInfo = async (retailerId) => {
    try {
        if (!(0, exports.checkIsNumber)(retailerId)) {
            return (0, exports.miniResponse)({
                status: false,
                others: { error: 'Retailer id not received or invalid' }
            });
        }
        const request = new mssql_1.default.Request();
        request.input('retail', mssql_1.default.Int, retailerId);
        const result = await request.query(`
            SELECT 
                rm.*,
...
exports.getRetailerInfo = getRetailerInfo;
const getProducts = async (IS_Sold = 1) => {
    try {
        const request = new mssql_1.default.Request();
        request.input('IS_Sold', mssql_1.default.Int, IS_Sold);
        const result = await request.query(`
            WITH RATE AS (
                SELECT * 
...
const getNextId = async ({ table, column }) => {
    try {
        ...
        const request = new mssql_1.default.Request();
        const result = await request.query(`SELECT ISNULL(MAX(${column}), 0) AS MaxId FROM ${table}`);
...
const getLargeData = async (exeQuery, db) => {
    try {
        const request = db ? new mssql_1.default.Request(db) : new mssql_1.default.Request();
        request.stream = true;
...
exports.getLargeData = getLargeData;
const getLOL = async (db) => {
    if (!db) {
        return (0, exports.miniResponse)({
            status: false,
    ...
    try {
        const result = await (0, exports.getLargeData)(`SELECT * FROM tbl_LOL_Excel ORDER BY id`, db);
        if (result.length > 0) {
            return (0, exports.miniResponse)({
                status: true,
                dataArray: result,
...
exports.getLOL = getLOL;
const getLOS = async (db) => {
    if (!db) {
        return (0, exports.miniResponse)({
            status: false,
            others: { error: 'Database connection pool is missing' }
    ...
    try {
        const result = await (0, exports.getLargeData)(`SELECT * FROM tbl_LOS_Excel ORDER BY id`, db);
        if (result.length > 0) {
            return (0, exports.miniResponse)({
                status: true,
                dataArray: result,
