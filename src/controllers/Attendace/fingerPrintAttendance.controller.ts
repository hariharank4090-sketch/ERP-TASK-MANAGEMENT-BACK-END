    ...
    try {
        ...
        if (EmpId && EmpId !== '0' && EmpId !== 'ALL') {
            result = await getSingleEmployeeAttendance(
                db, FromDate as string, adjustedToDateStr, EmpId as string
            );
        } else {
            result = await getMultipleEmployeesAttendance(
                db, FromDate as string, adjustedToDateStr, FingerPrintId as string | undefined
            );
        }

        return result.length > 0 ? dataFound(res, result) : noData(res);
...
export const getTodayAttendance = async (req: Request, res: Response): Promise<Response> => {
    try {
        ...
        const tomorrow = formatDateToString(
...
export const getEmployeeAttendanceSummary = async (req: Request, res: Response): Promise<Response> => {
    try {
        ...
        if (!EmpId) return invalidInput(res, 'EmpId is required');
        ...
        const toDateStr   = new Date(
            endDate.setDate(endDate.getDate() + 1)
        ).toISOString();

        const records = await getSingleEmployeeAttendance(
            db, fromDateStr, toDateStr, EmpId as string
        );
        ...
        const summary: AttendanceSummary = {
            ...
            attendancePercentage: records.length > 0
                ? Number(
                    (
                        (records.filter(r => r.AttendanceStatus === 'P').length / records.length) * 100
                    ).toFixed(2)
                  )
                : 0,
        };
...
export const getAbsentEmployees = async (req: Request, res: Response): Promise<Response> => {
    try {
        ...
        const adjustedTo = new Date(
            new Date(ToDate as string).setDate(new Date(ToDate as string).getDate() + 1)
        ).toISOString();

        const records = await getMultipleEmployeesAttendance(
            db, FromDate as string, adjustedTo
        );

        return dataFound(res, records.filter(r => r.AttendanceStatus === 'A'));
...
export const getPresentEmployees = async (req: Request, res: Response): Promise<Response> => {
    try {
        ...
        return dataFound(res, records.filter(r => r.AttendanceStatus === 'P'));
...
export const getAllDefaultLeaves = async (req: Request, res: Response): Promise<Response> => {
    try {
        const db = req.companyDB;
        if (!db) return servError(new Error('Company database connection not found'), res);

        const records = await db.query<DefaultLeaveResult>(
            `SELECT SNo, Date, Description, Created_By, Created_At, Modified_By, Modified_At
             FROM tbl_Default_Leave
             ORDER BY Date DESC, SNo DESC`,
            { type: QueryTypes.SELECT }
        );

        return records.length > 0 ? dataFound(res, records) : noData(res);
...
export const getDefaultLeavesByDateRange = async (req: Request, res: Response): Promise<Response> => {
    try {
        const db = req.companyDB;
        if (!db) return servError(new Error('Company database connection not found'), res);
...
export const getDefaultLeaveById = async (req: Request, res: Response): Promise<Response> => {
    try {
        const db = req.companyDB;
        if (!db) return servError(new Error('Company database connection not found'), res);

        const { id } = req.params;
        if (!id) return invalidInput(res, 'ID is required');
        ...
        return records.length > 0 ? dataFound(res, [records[0]]) : noData(res);
...
const getSingleEmployeeAttendance = async (
    ...
    return db.query<AttendanceResult>(query, {
...
const getMultipleEmployeesAttendance = async (
 ...
 WITH RankedLogs AS (
    ...
    const replacements: Record<string, unknown> = { FromDate, ToDate };
...
export const attendanceSync = async (req: Request, res: Response) => {
    try {
        ...
        await db.query(
            "EXEC Online_AttendanceLogs_SP @Start_Date = :startDate, @To_Date = :endDate",
            {
                replacements: { startDate, endDate }
            }
        );

        (dataFound as any)(res, [], "Attendance sync completed successfully", {
            startDate,
            endDate
        });
