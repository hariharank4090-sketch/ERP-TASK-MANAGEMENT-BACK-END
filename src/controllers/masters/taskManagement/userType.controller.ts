// src/controllers/masters/userType/userType.controller.ts
...
} from '../../../models/masters/users/userType.model';
 ...
 */
function requireCompanySequelize(req: Request) {
    if (!req.companyDB) {
 ...
 */
export const getAllUserTypes = async (req: Request, res: Response): Promise<Response> => {
    try {
        const companyDB     = requireCompanySequelize(req);
        const UserTypeModel = initUserTypeModel(companyDB);
 ...
 */
export const getUserTypeById = async (req: Request, res: Response): Promise<Response> => {
    try {
        const companyDB     = requireCompanySequelize(req);
        const UserTypeModel = initUserTypeModel(companyDB);
 ...
 */
export const createUserType = async (req: Request, res: Response): Promise<Response> => {
    try {
        const companyDB     = requireCompanySequelize(req);
        const UserTypeModel = initUserTypeModel(companyDB);

        // Validate body
        const parsed = userTypeCreateSchema.safeParse(req.body);
        if (!parsed.success) {
 ...
 * Body: { UserType?: string, Alias?: string, IsActive?: number }
 */
export const updateUserType = async (req: Request, res: Response): Promise<Response> => {
    try {
        const companyDB     = requireCompanySequelize(req);
        const UserTypeModel = initUserTypeModel(companyDB);
        const id            = Number(req.params.id);
        ...
        // Validate body
        const parsed = userTypeUpdateSchema.safeParse(req.body);
        if (!parsed.success) {
            // Properly handle ZodError format
            const errorMessages = parsed.error.issues.map(e => ({
                field:   e.path.join('.'),
 ...
 */
export const deleteUserType = async (req: Request, res: Response): Promise<Response> => {
    try {
        const companyDB     = requireCompanySequelize(req);
        const UserTypeModel = initUserTypeModel(companyDB);
        const id            = Number(req.params.id);
 ...
 */
export const getMyRole = async (req: Request, res: Response): Promise<Response> => {
    try {
        const companyDB     = requireCompanySequelize(req);
        const UserTypeModel = initUserTypeModel(companyDB);
